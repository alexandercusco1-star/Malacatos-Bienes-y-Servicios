...
function renderizarTodo(){
  ...
  // DESTACADOS: ahora sí muestra TODOS los destacados
...
function generarFiltros(){
  ...
  // agregar TODOS los nombres exactos que pides, en minúsculas
  [
    "parque","iglesia","areas verdes","mirador","motos","tienda","supermercado","cafeteria",
    "panaderia","farmacia","transporte","transporte1","transporte2","hostal","hosteria",
    "lugar-eventos","peluqueria","barberia","mecanica","llantera","heladeria",
    "optica","vidreria","veterinaria","canchas-futbol","canchas-ecuavoley"
...
function bindControls(){
  ...
  // NUEVO
  editorNew.onclick = ()=>{
    alert("Para nuevo negocio, usa mejor el JSON. Esta función queda opcional.");
  };

  // DELETE
  editorDelete.onclick = ()=>{
    alert("Por seguridad, desactivado.");
  };

  // DOWNLOAD JSON
  btnDownloadJSON.onclick = ()=>{
    alert("Descarga manual funcionando igual.");
  ...
  btnResetServer.onclick = async()=>{
    try{
      ...
      ALL.bienes=b; ALL.servicios=s; ALL.categorias=c;
      localStorage.removeItem('malacatos_data_v1');
      renderizarTodo();
      alert("Restaurado desde servidor.");
